...
fn main() {
	...
	render_app.add_render_graph_edges(
		Core3d,
		(Node3d::EndPrepasses, CopyDepthTexturePass, Node3d::MainOpaquePass),
	);
...
fn setup(
	...
	spawn_plane(&mut commands, &mut meshes, &mut show_depth_texture_materials, &demo_depth_texture);
...
fn spawn_plane(
	...
	let show_depth_texture_material = show_depth_texture_materials
		.add(ShowDepthTextureMaterial { depth_texture: Some(demo_depth_texture.0.clone()) });
	commands.spawn((
		Mesh3d(plane_handle),
...
impl SphericalCoordinates {
	...
	fn from_cartesian(p: Vec3) -> SphericalCoordinates {
		let radius = p.length();
		SphericalCoordinates { radius, inclination: acos(p.y / radius), azimuth: atan2(p.z, p.x) }
	}
	...
	fn to_cartesian(self) -> Vec3 {
		...
		self.radius
			* vec3(sin_inclination * cos_azimuth, cos_inclination, sin_inclination * sin_azimuth)
	}
}
