...
5. Append an ongoing dialogue log at the bottom of the same file as the agent answers questions or makes any changes. This log is scoped to the work since the current `HEAD` commit only; do not carry dialogue forward from older commit-message files after the developer commits. Keep the log concise: summarize the developer/agent back-and-forth rather than transcribing it. Use script notation, so the exchange is easy to scan: `- **Developer:** ...` and `- **Agent:** ...`.
