...
**Noisy shift along the boundary.** Jitter does **not** move the point into the cell interior or into an adjacent Basin Point Cell. It **slides** the intersection along the **boundary edge** it landed on: parameterize the edge by arc length $t \in [0,L]$, sample deterministic noise from $(\text{anchor}, \text{edge_{id}}, t)$ (or from the intersection coordinates quantized to a stable step), and add a signed offset $\Delta t$ along that edge. Reuse this same pattern anywhere you need **noisy boundary projection** (Basin Point Cell, Thalweg Cell, feature boundaries) so behavior stays consistent.
...
Done when: **short** stream overlays can attach to baked graph nodes under a **budget**, using existing geometry endpoints without a new global search, and remain subordinate to the inter-basin graph.