...
A grove that still uses `nest_placed_plant_chunk` will pay per-node hosts, per-type produce, and a larger visibility/transform wave. Flatten when the plants are kit instances (shared meshes) and fine-phase LOD on each stick/ball is not required. Collection merge (fewer plant hosts) is a later step; flattening is the first cut.