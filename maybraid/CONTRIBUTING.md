...
- **Presentation:** trees implement `VegetationComponents`. Isolated `/show` uses `FlattenedComponentsOnly<PlacedVegetation<Arc<T>>>` / `spawn_flattened_placed_vegetation`. Live groves emit posed kits under the grove host. Tuft groves without `LodScene` still use `ComponentsOnly` / `spawn_vegetation_components`.
