...
`PoiGoal`. `Downed` does the same until the corpse despawns. On Ignore, meander
is granted only while the tether is satisfied (or the NPC has no tether); an
unsatisfied leash or stalk drops `PoiGoal` so the member catches up, then
meander resumes. Retracting tether does not Hold in place, so firearm movement
and flee can write. Sticking to prey or a POI is a
