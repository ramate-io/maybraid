...
| `sight_nodes_for_level` | `sight_camera_socket` | [`guns/sights/`](../../art/items/guns/sights/) |
...
Hands bind to `grip_point` and `trigger_point` on the receiver, not to the grip or trigger-box meshes. Sights are the other attachment pattern: they socket onto `sight_camera_socket` with a rest scale for the 1 m authored cube, and they do not lengthen that bone. See the [guns README](../../art/items/guns/README.md) for the rest-pose tree and lengthening rules (`body` still parents `barrel` / `grip`).
