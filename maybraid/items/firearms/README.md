...
Muzzle is the barrel tail (`bone-local +Y` of rest length 1). Runtime rest (after the armature’s glTF +90° X) has bore along +Z and grip down; [`aim_plus_x`](src/pose.rs) yaws that onto world +X. Bolts and bullets are query-only; they sweep [`Fixed`](../../lod/avian/src/layers.rs) and charge [`Flight::through`](src/projectiles.rs) with optional [`PenetrationCost`](src/projectiles.rs). First contact with Fixed spawns a short Hanabi spark + smoke burst.
