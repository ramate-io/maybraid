...
pub enum Topple {
	/// Spawn a Topple through the resolved concepts pipeline.
	Preview(PreviewArgs),
}
...
pub struct PreviewArgs {
	#[arg(long, value_enum, default_value_t = ToppleBeakMesh::Beak)]
	...
	#[arg(long, value_enum, default_value_t = TopplePlumageColor::Cream)]
	pub plumage: TopplePlumageColor,

	#[arg(long, value_enum, default_value_t = ToppleEyeColor::SoftAmber)]
	pub eyes: ToppleEyeColor,

	#[arg(long, value_enum, default_value_t = ToppleBeakColor::Peach)]
	pub beak_color: ToppleBeakColor,
}
...
impl PreviewArgs {
	fn into_preview_config(self) -> ConceptPreviewConfig {
		let mut colors = crozon_characters::species::topple::ToppleColors::default();
		colors.plumage = self.plumage;
		colors.eyes = self.eyes;
		colors.beak = self.beak_color;
		ConceptPreviewConfig::topple_with_animation(
			ToppleConfig {
				beak: self.beak,
				eye: self.eye,
				hair: self.hair,
