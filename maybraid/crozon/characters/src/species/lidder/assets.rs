//! Lidder asset catalog and assembly resolver.
...
use crate::{
	...
	species::{
		...
		lidder::{pose::LidderPose, LidderConfig},
	},
};
...
/// Species-local resolver for Lidder asset choices.
pub struct LidderAssets;

impl LidderAssets {
	pub fn resolve(config: &LidderConfig) -> ResolvedCharacterAssembly {
	...
	fn beak(beak: LidderBeakMesh) -> ResolvedCharacterPart {
		ResolvedCharacterPart::new(
	...
	fn hair(hair: HairMesh) -> Option<ResolvedCharacterPart> {
		let path = hair.path()?;
		// Featherhawk is the Lidder crest: keep it small on the crown.
