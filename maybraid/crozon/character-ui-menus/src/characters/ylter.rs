...
use crozon_characters::{
	...
	species::ylter::{
		sliders::YilterSliders, YilterBodyMesh, YilterColors, YilterConfig, YilterMouthMesh,
	},
	ConceptAnimation,
};
...
/// Yilter quadruped body framing; also the species' default camera focus.
...
pub enum YilterAnimationClip {
	Still,
	Run,
	Gallop,
}

impl character_ui_menu::ListValues for YilterAnimationClip {
	fn values() -> &'static [Self] {
		&[Self::Still, Self::Run, Self::Gallop]
	}
}

impl character_ui_menu::LabelOption for YilterAnimationClip {
...
impl character_ui_menu::AssetOption for YilterAnimationClip {
...
impl From<YilterAnimationClip> for ConceptAnimation {
	fn from(value: YilterAnimationClip) -> Self {
		match value {
			YilterAnimationClip::Still => ConceptAnimation::Still,
			YilterAnimationClip::Run => ConceptAnimation::Run,
			YilterAnimationClip::Gallop => ConceptAnimation::Gallop,
		}
	}
}

impl From<ConceptAnimation> for YilterAnimationClip {
...
pub struct YilterAnimationMenu {
	pub clip: AssetSingleSelect<YilterAnimationClip>,
}

impl YilterAnimationMenu {
	pub fn new() -> Self {
		Self {
			clip: AssetSingleSelect::new(YilterAnimationClip::Still).with_camera_focus(BODY_FOCUS),
		}
	}
}

impl MenuComponent<MenuEvent> for YilterAnimationMenu {
...
#[derive(Clone, Debug, PartialEq)]
pub struct YilterHeadFeatureSliders {
...
#[derive(Clone, Debug, PartialEq)]
pub struct YilterBodyMenu {
	pub body: AssetSingleSelect<YilterBodyMesh>,
	pub sliders: YilterBodyProportionSliders,
	pub color: SwatchSingleSelect<ItemColor>,
	pub tail_color: SwatchSingleSelect<ItemColor>,
}

#[derive(Clone, Debug, PartialEq)]
pub struct YilterHeadFeaturesMenu {
	pub snout: AssetSingleSelect<YilterMouthMesh>,
	pub eye_color: SwatchSingleSelect<ItemColor>,
	pub mouth_color: SwatchSingleSelect<ItemColor>,
	pub feature_sliders: YilterHeadFeatureSliders,
	pub body_color: ItemColor,
}

#[derive(Clone, Debug, PartialEq)]
pub struct YilterMenu {
	pub presets: Section<YilterPresetsMenu>,
	pub body: Section<YilterBodyMenu>,
	pub head_features: Section<YilterHeadFeaturesMenu>,
	pub animation: Section<YilterAnimationMenu>,
}
...
impl YilterBodyProportionSliders {
	pub fn from_config(sliders: YilterSliders) -> Self {
		Self {
	...
	pub fn write_config(&self, sliders: &mut YilterSliders) {
...
impl MenuComponent<MenuEvent> for YilterBodyProportionSliders {
	fn menu_node(&self) -> MenuNode<MenuEvent> {
		MenuNode::fragment([
...
impl MenuComponent<MenuEvent> for YilterHeadFeatureSliders {
	fn menu_node(&self) -> MenuNode<MenuEvent> {
		MenuNode::fragment([
...
impl From<&YilterConfig> for YilterMenu {
	fn from(config: &YilterConfig) -> Self {
		Self {
			presets: Section::new(
				"Presets",
				YilterPresetsMenu {
			...
			head_features: Section::new(
				...
				YilterHeadFeaturesMenu {
					...
					feature_sliders: YilterHeadFeatureSliders::from_config(config.sliders),
					body_color: config.colors.body,
				},
			),
			animation: Section::new("Animation", YilterAnimationMenu::new()),
		}
	}
}

impl From<&YilterMenu> for YilterConfig {
	fn from(menu: &YilterMenu) -> Self {
		...
		Self {
			...
			sliders: {
				let mut sliders = YilterSliders::default();
...
impl MenuComponent<MenuEvent> for YilterPresetsMenu {
	fn menu_node(&self) -> MenuNode<MenuEvent> {
		MenuNode::fragment([
...
impl MenuComponent<MenuEvent> for YilterBodyMenu {
	fn menu_node(&self) -> MenuNode<MenuEvent> {
		MenuNode::fragment([
			MenuNode::asset_grid(
				"Body Mesh",
				...
				|value| {
					MenuEvent::SetAsset(CharacterField::YilterBody, AssetValue::YilterBody(value))
				},
			),
...
impl MenuComponent<MenuEvent> for YilterHeadFeaturesMenu {
	fn menu_node(&self) -> MenuNode<MenuEvent> {
		MenuNode::fragment([
			...
			MenuNode::asset_grid(
				...
				|value| {
					MenuEvent::SetAsset(CharacterField::YilterMouth, AssetValue::YilterMouth(value))
				},
			),
...
impl MenuComponent<MenuEvent> for YilterMenu {
	fn menu_node(&self) -> MenuNode<MenuEvent> {
		MenuNode::fragment([
...
impl YilterMenu {
	pub fn with_animation(mut self, animation: ConceptAnimation) -> Self {
		self.animation.value.clip.value = YilterAnimationClip::from(animation);
		self
	}
	...
	pub fn camera_focus_for_field(&self, field: CharacterField) -> Option<CameraFocus> {
		match field {
			CharacterField::YilterBody => self.body.value.body.camera_focus,
			CharacterField::YilterMouth => self.head_features.value.snout.camera_focus,
...
impl Default for YilterMenu {
	fn default() -> Self {
		Self::from(&YilterConfig::default_preview())
	}
}
