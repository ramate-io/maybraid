...
mod tests {
	...
	use super::*;
	use crate::animations::{FALL_SEGMENT_END, SPRING_SEGMENT_END, SQUAT_SEGMENT_END, Squat};

	#[test]
	fn spring_end_legs_straight() -> anyhow::Result<()> {
		let mut rig = HumanoidV0Rig::imported();
		let phase = SQUAT_SEGMENT_END + (SPRING_SEGMENT_END - SQUAT_SEGMENT_END) * 0.99;
	...
	fn fall_spreads_arms() -> anyhow::Result<()> {
		let mut rig = HumanoidV0Rig::imported();
		let phase = SPRING_SEGMENT_END + (FALL_SEGMENT_END - SPRING_SEGMENT_END) * 0.5;
		TwoFootedJump::<HumanoidV0Rig>::new(phase).apply(&mut rig);
	...
	fn land_peak_below_full_squat() -> anyhow::Result<()> {
		...
		let phase = FALL_SEGMENT_END + (1.0 - FALL_SEGMENT_END) * 0.5;
		TwoFootedJump::<HumanoidV0Rig>::new(phase).apply(&mut rig_land);
		let land_femur =
	...
	fn jump_effects_near_zero_at_cycle_boundaries() -> anyhow::Result<()> {
		let mut rig = HumanoidV0Rig::imported();
		let start = TwoFootedJump::<HumanoidV0Rig>::new(0.0).apply(&mut rig);
