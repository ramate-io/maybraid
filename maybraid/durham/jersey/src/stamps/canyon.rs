...
use crate::config::{FractalAnchors, HysteresisSpine, SoftmaskAlongSpine, MidpointGrading, DownhillPair};
...
impl Canyon {
	pub fn from_bounds(
		...
		let base_w = short * params.width_frac.clamp(0.04, 0.22) * params.confinement.clamp(0.4, 1.2);
		let noise = RegionNoise::from_seed(seed.wrapping_add(4), 0.03, base_w * 0.1);
		...
		match params.variant {
			...
			CanyonVariant::Chained => {
				...
				let s2 = (2 * n) / 3;
				let segments = [
					(&path[s0..=s1.min(n - 1)], base_w * 0.65, params.depth * 1.2),
					(&path[s1.min(n - 1)..=s2.min(n - 1)], base_w * 1.15, params.depth),
					(&path[s2.min(n - 1)..], base_w * 1.35, params.depth * 0.65),
				];
				for (seg, w, d) in segments {
					modulations.extend(SoftmaskAlongSpine::default().build(
						seg, w, 0.5, -d, 0.15, 0.55, &noise, Vec2::ZERO,
					));
				}
			}
		}
