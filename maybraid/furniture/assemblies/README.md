...
Chico-shaped furniture kits: `BedParams` / `ChairParams` / `ChestParams` / `CounterParams` / sit-on and kitchen params → `build()` → exploded parts. `unit_from_num` changes [`MaterialRef`](../../material-ref/) palette only.
