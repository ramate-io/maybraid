...
use crate::theme::{
	PANEL_CURSOR_ICON_SIZE, PANEL_GROUP_FONT_SIZE, PANEL_HEADER_FONT_SIZE, PANEL_ITEM_FONT_SIZE,
	TEXT_YELLOW, TEXT_YELLOW_FAINT,
};

use super::HudFonts;

/// Block title above a section body or asset grid.
...
pub fn spawn_cursor_slot(parent: &mut ChildSpawnerCommands, fonts: &HudFonts, visible: bool) {
	...
	parent
		.spawn((
			TextCursorSlot,
			Node {
				width: Val::Px(PANEL_CURSOR_ICON_SIZE),
				height: Val::Px(PANEL_CURSOR_ICON_SIZE),
				flex_shrink: 0.0,
				..default()
			},
			Pickable::IGNORE,
		...
		.with_children(|slot| {
			let (icon, animated) = AnimatedIcon::maybraid(PANEL_CURSOR_ICON_SIZE, TEXT_YELLOW);
