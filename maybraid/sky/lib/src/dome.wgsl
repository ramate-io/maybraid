...
fn haze_cover(t: f32, phase: f32) -> f32 {
    let a = fbm2(vec2<f32>(t * 0.028 + phase * 3.0, 2.17));
    let b = fbm2(vec2<f32>(-t * 0.022 + phase, 8.41));
    return mix(0.28, 0.78, smoothstep(0.32, 0.62, a * 0.6 + b * 0.4));
}

fn haze_island(dir: vec3<f32>, t: f32) -> f32 {
    let p = dir * 1.45 + vec3<f32>(t * 0.055, -t * 0.048, t * 0.041);
    let warp = fbm3(p);
    let n = fbm3(p * 1.1 + vec3<f32>(warp * 0.55, t * 0.038, -warp));
    let n2 = fbm3(p * 1.45 + vec3<f32>(-t * 0.044, warp, t * 0.033));
    return saturate(n * 0.62 + n2 * 0.38);
}

fn cosmos_hole(dir: vec3<f32>, t: f32) -> f32 {
    let p = dir * 1.35 + vec3<f32>(3.7, -1.4, t * 0.048);
    let n = fbm3(p + vec3<f32>(t * 0.042, -t * 0.036, 0.6));
    let n2 = fbm3(p * 1.35 + vec3<f32>(-t * 0.031, t * 0.046, 2.2));
...
/// One logo shard. `layer` 0 is the main ring; 1 is smaller chips in the gaps.
...
/// Detached shards around the full rim, plus a chip ring in the leftover gaps.
