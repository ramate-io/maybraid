...
- Every presenting 100 m tile softmax-blends its cardinal neighbor **grove** slots ([`blend.rs`](src/blend.rs)): distance is the likelihood, a spatial logit wrinkle plus the CDF hash make the front jagged. An empty layer still presents tiles when a neighbor slot has a kind (islands). Forest cells only choose each slot's kind and, for streaming, which selections to cache (`R+1` halo, grow `R`). See [`assemble`](src/assemble.rs) + [`NeighborLayers`](src/assemble.rs).
