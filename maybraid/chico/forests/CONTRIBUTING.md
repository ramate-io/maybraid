...
- Canopy bump-outs read [`BumpOutSelection`](src/bump_out.rs) on [`ForestGroveKind`](src/kind.rs) (authored density / bite / height / palette). Sample the **highest occupied layer**. Blend overlapping 100 m tiles onto the 160 m terrain cell. Do not grow groves for bump-outs. [`CanopyBumpOut`](src/bump_out.rs) `build_with_id` `get_or_generate`s overlapping [`ChicoForest`](src/forest.rs) cells, then stores the blended 3×3 neighborhood.
...
- Forest present spawns [`ChicoGroveHost`](src/host.rs) (`ForestGroveTile` + [`ForestLayer`](src/kind.rs)), not the concrete grove type ([#652](https://github.com/ramate-io/maybraid/issues/652)). Register one `avian_host!(ChicoGroveHost)` on the forest / world path. Typed `/show orchard` still uses the concrete grove `LodScene`.
...
- Default grove stream rings: generate [`GROVE_GENERATE_RADIUS_M`](src/generation.rs) (3 km, selection / recipes only), present [`GROVE_PRESENT_RADIUS_M`](src/generation.rs) (1 km, grown geometry). Canopy bump-outs present a 1–3 km annulus ([`BUMP_OUT_INNER_RADIUS_M`](src/bump_out.rs)–[`BUMP_OUT_OUTER_RADIUS_M`](src/bump_out.rs)) of 160 m cells so they do not cover filled groves.
