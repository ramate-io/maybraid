//! Terminal canopy: compact [`PlaneSplay`] on upper/outer nodes, [`ChicoBall`] elsewhere ([#248](https://github.com/ramate-io/maybraid/issues/248)).
...
fn should_allocate_foliage(
	node_idx: usize,
	hysteresis: &PenmarchTorchChain,
	chain: &BallStickChain<PenmarchTorchChain>,
) -> bool {
...
fn prefers_plane_splay(hysteresis: &PenmarchTorchChain, node_idx: usize, position: Vec3) -> bool {
...
#[derive(Clone)]
pub(crate) struct PenmarchTorchLeafCanopyRule<LeafM, LeafS>
where
	LeafM: Material,
...
impl<LeafM, LeafS> BallRenderRule<LayeredTerminalCanopyItem<LeafM, LeafS>, PenmarchTorchChain>
	for PenmarchTorchLeafCanopyRule<LeafM, LeafS>
where
