...
use chico_sbs_trees::simplemans_hedge::SimplemansHedgeParams;
		...
		for placed in self.placements() {
			...
			let entities = match placed.variant.item() {
				...
				LevantineScrubItem::Hedge(hedge) => {
					...
					let params = SimplemansHedgeParams::new(
						samples.height,
...
mod tests {
	...
	fn rory_vase_bush_torch_and_hedge_geometry_build_within_authored_ranges() -> Result<()> {
		...
		assert_eq!(params.clump_anchors().len(), params.clump_count as usize);
		Ok(())
	}
