...
mod vc {
	...
	pub const TEMPERATE_MASSIVES_STRUCTURAL_MEDIUM_FACTOR: f32 = 5.0;
	pub const TEMPERATE_MASSIVES_STRUCTURAL_LOW_FACTOR: f32 = 20.0;
	...
	pub struct TemperateMassives {
		pub plants: Vec<TemperateMassivesPlant>,
		...
		fn nest_plant_chunks(&self, lod_ref: &LodRef) -> Vec<SceneChunk> {
			self.plants
				...
				.map(|plant| match &plant.kind {
					TemperateMassivesKind::Oak(t) => nest_placed_plant_chunk(
						t.clone(),
						plant.placement,
					...
					TemperateMassivesKind::Storybook(t) => nest_placed_plant_chunk(
						t.clone(),
						plant.placement,
						&plant.stick_material,
						&plant.ball_material,
					...
					TemperateMassivesKind::Rory(t) => nest_placed_plant_chunk(
						t.clone(),
						plant.placement,
						&plant.stick_material,
						&plant.ball_material,
	...
	fn grow_plant(
		...
		let kind = match placed.variant.item() {
			...
			TemperateMassivesItem::Storybook(story) => {
				...
				params.geometry = geometry;
				TemperateMassivesKind::Storybook(params.build())
			}
			TemperateMassivesItem::Rory(rory) => {
				...
				params.geometry = geometry;
				TemperateMassivesKind::Rory(params.build())
			}
		};
