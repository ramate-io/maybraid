...
mod vc {
		...
		match placed.variant.item() {
			RiparianGeneralItem::BraidOak(oak) => {
				...
				RiparianGeneralPlant {
					...
					kind: RiparianGeneralKind::Oak(Arc::new(unit_params.build())),
					stick_material,
					ball_material,
					frond_material,
				}
	...
	mod tests {
		...
		fn high_medium_nest_one_flattened_host_per_tree() -> Result<()> {
			...
			assert!(!grove.plants.is_empty(), "expected placed riparian-general plants");
			...
			let lod::SceneChunk::SubChunks(parts) = high else {
				anyhow::bail!("High riparian-general should wrap plant chunks");
			};
			...
			let lod::SceneChunk::Lazy { remaining_primitives, remaining_weight, .. } = &parts[0]
				...
				anyhow::bail!("High riparian-general plants should be SceneChunk::Lazy");
			};
