...
pub type ThicketHonu = HonuBanyan<
	ChicoStickMaterial,
	...
	Terrain: TerrainSample + Clone + Send + Sync + 'static + Default + clap::Args,
{
	...
	#[arg(skip)]
	resolved_placements: Option<Vec<GrovePlacedCell<TropicalThicketCell>>>,
	...
	Terrain: TerrainSample + Clone + Send + Sync + 'static + Default + clap::Args,
{
	pub fn with_resolved_placements(
		resolved_placements: Vec<GrovePlacedCell<TropicalThicketCell>>,
		terrain: Terrain,
	...
	pub fn placements(&self) -> Vec<GrovePlacedCell<TropicalThicketCell>> {
...
impl BuildWithNoise<HighBushShootsShape> for TropicalThicketBush {
		...
		for placed in self.placements() {
			...
			let entities = match placed.variant.item() {
				...
				TropicalThicketItem::Banyan(banyan) => {
					...
					let stick_seed =
						placement_noise(self.bush_chain_noise, placed.position).seed as i32;
					let canopy_seed = build_noise.seed as i32 + 31;
