//! [`RenderItem`] for populated Dryland groves ([#335](https://github.com/ramate-io/maybraid/issues/335)).

use std::marker::PhantomData;

use bevy::prelude::*;
use chico_sbs_geometry::{LiamsConiferSbs, VaseTreeSbs};
...
use crate::dryland::{definition, DrylandCell, DrylandItem, DrylandLiamsConifer, DrylandVaseTree};
use crate::grove::{
...
/// Dryland grove preview (sparse Liam's Conifer and Vase Tree arid upper-canopy forms).
#[derive(Clone, Args)]
#[command(rename_all = "kebab-case")]
pub struct Dryland<StickM, StickS, LeafM, LeafS, Terrain = FlatTerrainSample>
where
	StickM: Material,
	...
	Terrain: TerrainSample + Clone + Send + Sync + 'static + Default + clap::Args,
{
	...
	#[arg(skip)]
	resolved_placements: Option<Vec<GrovePlacedCell<DrylandCell>>>,

	#[arg(skip)]
	__marker: PhantomData<fn() -> (StickM, LeafM)>,
}

impl<StickM, StickS, LeafM, LeafS, Terrain> Default for Dryland<StickM, StickS, LeafM, LeafS, Terrain>
where
	StickM: Material,
...
impl<StickM, StickS, LeafM, LeafS, Terrain> Dryland<StickM, StickS, LeafM, LeafS, Terrain>
where
	StickM: Material,
	StickS: Clone + Into<MeshMaterial3d<StickM>> + Args + Send + Sync + 'static,
	LeafM: Material,
	...
	Terrain: TerrainSample + Clone + Send + Sync + 'static + Default + clap::Args,
{
	pub fn with_resolved_placements(
		resolved_placements: Vec<GrovePlacedCell<DrylandCell>>,
		terrain: Terrain,
	...
	pub fn placements(&self) -> Vec<GrovePlacedCell<DrylandCell>> {
...
impl BuildWithNoise<VaseTreeSbs> for DrylandVaseTree {
...
impl<StickM, StickS, LeafM, LeafS, Terrain> RenderItem
	for Dryland<StickM, StickS, LeafM, LeafS, Terrain>
where
...
mod tests {
	...
	fn tree_geometry_builds_within_authored_ranges() -> Result<()> {
		...
		let liams_geom = liams.build_with_noise(noise);
		assert!(liams_geom.scale.stalk_height >= liams.height.start.min(liams.height.end));
		assert!(liams_geom.scale.stalk_height <= liams.height.start.max(liams.height.end));
		...
		assert!(vase_geom.scale.tree_height >= vase.height.start.min(vase.height.end));
		assert!(vase_geom.scale.tree_height <= vase.height.start.max(vase.height.end));
		Ok(())
	}
	...
	fn with_resolved_placements_skips_live_selection() -> Result<()> {
		let placement =
			GrovePlacedCell::new(DrylandCell::DrylandLiamsConifer, Vec3::new(1.0, 0.0, 2.0), 1.0, Cell::from_min_max(Vec3::new(0.0, 0.0, 0.0), Vec3::new(1.0, 1.0, 1.0)));
		let item = DrylandStd::with_resolved_placements(
			vec![placement.clone()],
