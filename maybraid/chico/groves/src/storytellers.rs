...
mod vc {
	...
	use chico_sbs_trees::{BraidOakTree, BraidOakTreeParams, KamakuraTorch, KamakuraTorchParams, PenmarchTorch, PenmarchTorchParams, StorybookTree, StorybookTreeParams};
	...
	pub struct Storytellers {
		pub plants: Vec<StorytellersPlant>,
		...
		fn nest_plant_chunks(&self, lod_ref: &LodRef) -> Vec<SceneChunk> {
			self.plants
				...
				.map(|plant| match &plant.kind {
					StorytellersKind::Oak(t) => nest_placed_plant_chunk(
						t.clone(),
						plant.placement,
					...
					StorytellersKind::Storybook(t) => nest_placed_plant_chunk(
						t.clone(),
						plant.placement,
						&plant.stick_material,
						&plant.ball_material,
					...
					StorytellersKind::Penmarch(t) => nest_placed_plant_chunk(
						t.clone(),
						plant.placement,
						&plant.stick_material,
						&plant.ball_material,
					...
					StorytellersKind::Kamakura(t) => nest_placed_plant_chunk(
						t.clone(),
						plant.placement,
						&plant.stick_material,
						&plant.ball_material,
	...
	fn grow_plant(
		...
		let kind = match placed.variant.item() {
			StorytellersItem::BraidOak(oak) => {
				...
				StorytellersKind::Oak(params.build())
			}
			StorytellersItem::Storybook(story) => {
				...
				params.geometry = geometry;
				StorytellersKind::Storybook(params.build())
			}
			StorytellersItem::PenmarchTorch(torch) | StorytellersItem::TorchTree(torch) => {
				...
				params.geometry = geometry;
				StorytellersKind::Penmarch(params.build())
			}
			StorytellersItem::KamakuraTorch(torch) => {
				...
				params.geometry = geometry;
				StorytellersKind::Kamakura(params.build())
			}
		};
