//! [`RenderItem`] for populated Temperate Lower Massives groves ([#330](https://github.com/ramate-io/maybraid/issues/330)).
...
use crate::temperate_lower_massives::{
	definition, TemperateLowerMassivesBraidOak, TemperateLowerMassivesCell,
	TemperateLowerMassivesItem, TemperateLowerMassivesRory, TemperateLowerMassivesStorybook,
};
...
/// Temperate Lower Massives grove preview (10–20 m temperate massives beneath upper canopy).
#[derive(Clone, Args)]
#[command(rename_all = "kebab-case")]
pub struct TemperateLowerMassives<StickM, StickS, LeafM, LeafS, Terrain = FlatTerrainSample>
where
	StickM: Material,
...
impl<StickM, StickS, LeafM, LeafS, Terrain> Default
	for TemperateLowerMassives<StickM, StickS, LeafM, LeafS, Terrain>
where
	StickM: Material,
...
impl<StickM, StickS, LeafM, LeafS, Terrain>
	TemperateLowerMassives<StickM, StickS, LeafM, LeafS, Terrain>
where
	StickM: Material,
	StickS: Clone + Into<MeshMaterial3d<StickM>> + Args + Send + Sync + 'static,
	LeafM: Material,
...
impl BuildWithNoise<BraidOakTreeSbs> for TemperateLowerMassivesBraidOak {
...
impl BuildWithNoise<StorybookTreeSbs> for TemperateLowerMassivesStorybook {
...
impl BuildWithNoise<RorysHeadTrainedSbs> for TemperateLowerMassivesRory {
...
mod tests {
	...
	fn default_weights_yield_low_density_placements_in_preview_grid() -> Result<()> {
		...
		let grove = TemperateLowerMassivesStd::default()
		...
		assert!(
			(0.10..=0.36).contains(&placed_share),
			"expected temperate-lower-massives fill, got {placed_share} ({}/{cells})",
			placements.len()
		);
		Ok(())
	}
