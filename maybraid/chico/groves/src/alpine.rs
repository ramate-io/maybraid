...
mod vc {
	...
	mod tests {
		...
		fn high_medium_nest_one_flattened_host_per_tree() -> Result<()> {
			...
			assert!(!grove.plants.is_empty(), "expected placed alpine plants");
			...
			let lod::SceneChunk::SubChunks(parts) = high else {
				anyhow::bail!("High alpine should wrap plant chunks");
			};
			...
			let lod::SceneChunk::Lazy { remaining_primitives, remaining_weight, .. } = &parts[0]
				...
				anyhow::bail!("High alpine plants should be SceneChunk::Lazy");
			};
