...
mod vc {
	...
	pub const CONIFER_MASSIVES_STRUCTURAL_HIGH_FACTOR: f32 = 5.0;
	pub const CONIFER_MASSIVES_STRUCTURAL_MEDIUM_FACTOR: f32 = 20.0;
	pub const CONIFER_MASSIVES_STRUCTURAL_LOW_FACTOR: f32 = 30.0;
	...
	mod tests {
		...
		fn high_medium_nest_one_flattened_host_per_tree() -> Result<()> {
			...
			assert!(!grove.plants.is_empty(), "expected placed conifer-massives plants");
			...
			let lod::SceneChunk::SubChunks(parts) = high else {
				anyhow::bail!("High conifer-massives should wrap plant chunks");
			};
			...
			let lod::SceneChunk::Lazy { remaining_primitives, remaining_weight, .. } = &parts[0]
				...
				anyhow::bail!("High conifer-massives plants should be SceneChunk::Lazy");
			};
