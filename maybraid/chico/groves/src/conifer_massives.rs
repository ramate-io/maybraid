...
mod vc {
	...
	use chico_sbs_trees::{FriendsConifer, FriendsConiferParams, LiamsConifer, LiamsConiferParams, NorthernConifer, NorthernConiferParams, TemperateConifer, TemperateConiferParams};
	...
	impl ConiferMassivesParams {
		...
		pub fn build(&self) -> ConiferMassives {
			ConiferMassives::from_placements(&self.placements(), self.grove.noise, &self.extent)
		}
	}
	...
	pub struct ConiferMassives {
		pub plants: Vec<ConiferMassivesPlant>,
		...
		fn nest_plant_chunks(&self, lod_ref: &LodRef) -> Vec<SceneChunk> {
			self.plants
				...
				.map(|plant| match &plant.kind {
					ConiferMassivesKind::Northern(t) => nest_placed_plant_chunk(
						t.clone(),
						plant.placement,
					...
					ConiferMassivesKind::Friends(t) => nest_placed_plant_chunk(
						t.clone(),
						plant.placement,
						&plant.stick_material,
						&plant.ball_material,
					...
					ConiferMassivesKind::Liams(t) => nest_placed_plant_chunk(
						t.clone(),
						plant.placement,
						&plant.stick_material,
						&plant.ball_material,
					...
					ConiferMassivesKind::Temperate(t) => nest_placed_plant_chunk(
						t.clone(),
						plant.placement,
						&plant.stick_material,
						&plant.ball_material,
	...
	fn grow_plant(
		...
		let kind = match placed.variant.item() {
			ConiferMassivesItem::NorthernConifer(conifer) => {
				...
				ConiferMassivesKind::Northern(params.build())
			}
			ConiferMassivesItem::FriendsConifer(conifer) => {
				...
				params.splay_radius_fraction_of_height = samples.splay_radius_fraction_of_height;
				params.apex_canopy_spawn_fraction = samples.apex_canopy_spawn_fraction;
				ConiferMassivesKind::Friends(params.build())
			}
			ConiferMassivesItem::LiamsConifer(conifer) => {
				...
				ConiferMassivesKind::Liams(params.build())
			}
			ConiferMassivesItem::TemperateConifer(temperate) => {
				...
				params.apex_canopy_spawn_fraction = samples.apex_canopy_spawn_fraction;
				ConiferMassivesKind::Temperate(params.build())
			}
		};
	...
	impl VegetationComponents for ConiferMassives {
	...
	impl LodScene for ConiferMassives {
...
#[cfg(feature = "render")]
pub use vc::{
	ConiferMassives, ConiferMassivesParams, ConiferMassivesPlant, CONIFER_MASSIVES_STRUCTURAL_HIGH_FACTOR,
	CONIFER_MASSIVES_STRUCTURAL_LOW_FACTOR, CONIFER_MASSIVES_STRUCTURAL_MEDIUM_FACTOR,
};
