...
mod vc {
	...
	use chico_sbs_trees::{StorybookTree, StorybookTreeParams, TemperateConifer, TemperateConiferParams};
	...
	pub const LEEWARD_STRUCTURAL_HIGH_FACTOR: f32 = 2.0;
	pub const LEEWARD_STRUCTURAL_MEDIUM_FACTOR: f32 = 5.0;
	...
	impl LeewardParams {
		...
		pub fn build(&self) -> Leeward {
			Leeward::from_placements(&self.placements(), self.grove.noise, &self.extent)
		}
	}
		...
		fn nest_plant_chunks(&self, lod_ref: &LodRef) -> Vec<SceneChunk> {
			self.plants
				...
				.map(|plant| match &plant.kind {
					LeewardKind::Storybook(t) => nest_placed_plant_chunk(
						t.clone(),
						plant.placement,
					...
					LeewardKind::Temperate(t) => nest_placed_plant_chunk(
						t.clone(),
						plant.placement,
						&plant.stick_material,
						&plant.ball_material,
	...
	fn grow_plant(
		...
		let kind = match placed.variant.item() {
			LeewardItem::Storybook(story) => {
				...
				LeewardKind::Storybook(params.build())
			}
			LeewardItem::TemperateConifer(temperate) => {
				...
				LeewardKind::Temperate(params.build())
			}
		};
