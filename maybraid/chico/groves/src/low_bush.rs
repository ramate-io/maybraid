...
mod vc {
	...
	use super::{definition, LowBushCell, LowBushItem};
	...
	pub const LOW_BUSH_STRUCTURAL_HIGH_FACTOR: f32 = 2.0;
	pub const LOW_BUSH_STRUCTURAL_MEDIUM_FACTOR: f32 = 5.0;
	...
	impl Default for LowBushParams {
		fn default() -> Self {
			Self {
				...
				terrain: FlatTerrainSample { elevation: 0.30, steepness: 0.15 },
				tree_variants: 100,
	...
	impl LowBushParams {
		...
		) -> Vec<GroveCellVariant<LowBushCell>> {
			if let Some(ref resolved) = self.resolved_placements {
				return resolved.clone();
			}
	...
	fn grow_plant(
		...
		let LowBushItem::Bush(bush) = placed.variant.item();
	...
	impl VegetationComponents for LowBush {
	...
	mod tests {
		...
		fn small_grove() -> LowBush {
			LowBushParams::default()
		...
		fn high_medium_nest_one_flattened_host_per_tree() -> Result<()> {
			...
			assert!(!grove.plants.is_empty(), "expected placed low bushes");
			...
			let lod::SceneChunk::SubChunks(parts) = high else {
				anyhow::bail!("High low-bush should wrap plant chunks");
			};
			...
			let lod::SceneChunk::Lazy { remaining_primitives, remaining_weight, .. } = &parts[0]
				...
				anyhow::bail!("High low-bush plants should be SceneChunk::Lazy");
			};
			...
			let lod::SceneChunk::Primitive { weight, .. } =
				...
				anyhow::bail!("Low low-bush should emit one flattened canopy collection");
			};
		...
		fn tree_variants_quantize_archetypes() -> Result<()> {
			...
			let mut params = LowBushParams::default()
				.with_extent(GroveExtent::new(Vec3::ZERO, Vec3::new(40.0, 1.0, 40.0)));
...
#[cfg(feature = "render")]
pub use vc::{
	LowBush, LowBushParams, LowBushPlant, LOW_BUSH_STRUCTURAL_HIGH_FACTOR,
	LOW_BUSH_STRUCTURAL_LOW_FACTOR, LOW_BUSH_STRUCTURAL_MEDIUM_FACTOR,
};
