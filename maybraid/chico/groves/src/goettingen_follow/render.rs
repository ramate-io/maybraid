...
use crate::goettingen_follow::{
	definition, GoettingenFollowBraidOak, GoettingenFollowCell, GoettingenFollowItem,
	...
	Terrain: TerrainSample + Clone + Send + Sync + 'static + Default + clap::Args,
{
	...
	#[arg(skip)]
	resolved_placements: Option<Vec<GrovePlacedCell<GoettingenFollowCell>>>,
	...
	Terrain: TerrainSample + Clone + Send + Sync + 'static + Default + clap::Args,
{
	pub fn with_resolved_placements(
		resolved_placements: Vec<GrovePlacedCell<GoettingenFollowCell>>,
		terrain: Terrain,
	...
	pub fn placements(&self) -> Vec<GrovePlacedCell<GoettingenFollowCell>> {
...
const FOLLOW_RING_SPACING_SCALE: f32 = 1.25;
const FOLLOW_ANCHORS_PER_RING: u32 = 5;

fn follow_ring_spacing(base: f32) -> f32 {
	base * FOLLOW_RING_SPACING_SCALE
}

impl BuildWithNoise<BraidOakTreeSbs> for GoettingenFollowBraidOak {
...
impl BuildWithNoise<StorybookTreeSbs> for GoettingenFollowStorybook {
	fn build_with_noise(&self, noise: NoiseParams) -> StorybookTreeSbs {
		...
		geometry.rings.spacing = follow_ring_spacing(geometry.rings.spacing);
		geometry.rings.anchors_per_ring =
			FOLLOW_ANCHORS_PER_RING + (canopy_density * 2.0).round() as u32;
