//! [`RenderItem`] for populated Low Bush groves ([#310](https://github.com/ramate-io/maybraid/issues/310)).
...
/// Typical [`ChicoStickMaterial`] / [`StandardMaterial`] Low Bush instance.
pub type LowBushStd = LowBush<
	ChicoStickMaterial,
...
/// Low Bush grove preview (stick + leaf materials → Common High Bush shrubs).
#[derive(Clone, Args)]
#[command(rename_all = "kebab-case")]
pub struct LowBush<StickM, StickS, LeafM, LeafS, Terrain = FlatTerrainSample>
where
	StickM: Material,
...
impl<StickM, StickS, LeafM, LeafS, Terrain> Default
	for LowBush<StickM, StickS, LeafM, LeafS, Terrain>
where
	StickM: Material,
...
impl<StickM, StickS, LeafM, LeafS, Terrain> LowBush<StickM, StickS, LeafM, LeafS, Terrain>
where
	StickM: Material,
	StickS: Clone + Into<MeshMaterial3d<StickM>> + Args + Send + Sync + 'static,
	LeafM: Material,
...
impl BuildWithNoise<HighBushShootsShape> for LowBushBush {
	fn build_with_noise(&self, noise: NoiseParams) -> HighBushShootsShape {
		...
		let height = sample_f32(self.height, 1.0).max(0.35);
...
mod tests {
	...
	fn with_resolved_placements_skips_live_selection() -> Result<()> {
		let placement =
			GrovePlacedCell::new(LowBushCell::GreenLowBush, Vec3::new(1.0, 0.0, 2.0), 1.0);
	...
	fn default_weights_yield_moderate_placements_in_preview_grid() -> Result<()> {
		...
		assert!(
			(0.18..=0.45).contains(&placed_share),
			"expected moderate low-bush fill, got {placed_share} ({}/{cells})",
			placements.len()
		);
