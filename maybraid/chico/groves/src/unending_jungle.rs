		...
		match placed.variant.item() {
			UnendingJungleItem::Honu(banyan) => {
				...
				UnendingJunglePlant {
					...
					kind: UnendingJungleKind::Honu(HonuBanyan::grow_num(variant).0),
					stick_material,
					ball_material,
					frond_material,
				}
			...
			UnendingJungleItem::Sope(banyan) => {
				...
				UnendingJunglePlant {
					placement: Placement::new(placed.position, 0.0)
						.with_scale(Vec3::splat((placed.scale * world_size).max(1e-4))),
					kind: UnendingJungleKind::Sope(SopesBanyan::grow_num(variant).0),
					stick_material,
					ball_material,
					frond_material,
				}
			...
			UnendingJungleItem::JungleStorybook(jungle) => {
				...
				UnendingJunglePlant {
					placement: Placement::new(placed.position, 0.0)
						.with_scale(Vec3::splat((placed.scale * world_size).max(1e-4))),
					kind: UnendingJungleKind::JungleStorybook(
