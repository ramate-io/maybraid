...
mod vc {
	...
	use crate::grove::{
		canopy_ball_material_from_palette, canopy_proxy_rory, frond_material_from_palette, grove_structural_footprint, nest_flattened_plant_chunk, placement_noise, remixed_sbs_plant,
	...
	mod tests {
		...
		fn high_medium_nest_one_flattened_host_per_tree() -> Result<()> {
			...
			assert!(!grove.plants.is_empty(), "expected placed vineyard vines");
			...
			let lod::SceneChunk::SubChunks(parts) = high else {
				anyhow::bail!("High vineyard should wrap plant chunks");
			};
			...
			let lod::SceneChunk::Lazy { remaining_primitives, remaining_weight, .. } = &parts[0]
				...
				anyhow::bail!("High vineyard plants should be SceneChunk::Lazy");
			};
