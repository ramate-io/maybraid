...
mod vc {
	...
	pub struct Shamanhome {
		pub plants: Vec<ShamanhomePlant>,
		...
		fn nest_plant_chunks(&self, lod_ref: &LodRef) -> Vec<SceneChunk> {
			self.plants
				...
				.map(|plant| match &plant.kind {
					ShamanhomeKind::Oak(t) => nest_placed_plant_chunk(
						t.clone(),
						plant.placement,
					...
					ShamanhomeKind::Date(t) => nest_placed_plant_chunk(
						t.clone(),
						plant.placement,
						&plant.stick_material,
						&plant.ball_material,
					...
					ShamanhomeKind::Sope(t) => nest_placed_plant_chunk(
						t.clone(),
						plant.placement,
						&plant.stick_material,
						&plant.ball_material,
			...
			self.plants
				...
				.filter_map(|plant| {
					...
					match &plant.kind {
						...
						ShamanhomeKind::Date(t) => canopy_proxy_site(t, plant.placement, material),
	...
	fn grow_plant(
		...
		let kind = match placed.variant.item() {
			...
			ShamanhomeItem::SopeBanyan(banyan) => {
				...
				ShamanhomeKind::Sope(params.build())
			}
		};

		ShamanhomePlant { placement, kind, stick_material, ball_material, frond_material }
	}
