...
mod vc {
	...
	pub struct TemperateLowerMassives {
		pub plants: Vec<TemperateLowerMassivesPlant>,
		...
		fn nest_plant_chunks(&self, lod_ref: &LodRef) -> Vec<SceneChunk> {
			self.plants
				...
				.map(|plant| match &plant.kind {
					TemperateLowerMassivesKind::Oak(t) => nest_placed_plant_chunk(
						t.clone(),
						plant.placement,
					...
					TemperateLowerMassivesKind::Storybook(t) => nest_placed_plant_chunk(
						t.clone(),
						plant.placement,
						&plant.stick_material,
						&plant.ball_material,
					...
					TemperateLowerMassivesKind::Rory(t) => nest_placed_plant_chunk(
						t.clone(),
						plant.placement,
						&plant.stick_material,
						&plant.ball_material,
	...
	fn grow_plant(
		...
		let kind = match placed.variant.item() {
			TemperateLowerMassivesItem::BraidOak(oak) => {
				...
				TemperateLowerMassivesKind::Oak(params.build())
			}
			TemperateLowerMassivesItem::Storybook(story) => {
				...
				params.geometry = geometry;
				TemperateLowerMassivesKind::Storybook(params.build())
			}
			TemperateLowerMassivesItem::Rory(rory) => {
				...
				params.geometry = geometry;
				TemperateLowerMassivesKind::Rory(params.build())
			}
		};
...
#[cfg(feature = "render")]
pub use vc::{
	TemperateLowerMassives, TemperateLowerMassivesParams, TemperateLowerMassivesPlant, TEMPERATE_LOWER_MASSIVES_STRUCTURAL_HIGH_FACTOR,
	TEMPERATE_LOWER_MASSIVES_STRUCTURAL_LOW_FACTOR, TEMPERATE_LOWER_MASSIVES_STRUCTURAL_MEDIUM_FACTOR,
};
