...
mod vc {
	...
	pub struct RollingOaks {
		pub plants: Vec<RollingOaksPlant>,
		...
		fn nest_plant_chunks(&self, lod_ref: &LodRef) -> Vec<SceneChunk> {
			self.plants
				...
				.map(|plant| match &plant.kind {
					RollingOaksKind::Oak(t) => nest_placed_plant_chunk(
						t.clone(),
						plant.placement,
					...
					RollingOaksKind::Storybook(t) => nest_placed_plant_chunk(
						t.clone(),
						plant.placement,
						&plant.stick_material,
						&plant.ball_material,
	...
	fn grow_plant(
		...
		let kind = match placed.variant.item() {
			RollingOaksItem::BraidOak(oak) => {
				...
				RollingOaksKind::Oak(params.build())
			}
			RollingOaksItem::Storybook(story) => {
				let geometry = story.build_with_noise(build_noise);
				let mut params = StorybookTreeParams::default();
				params.geometry = geometry;
				RollingOaksKind::Storybook(params.build())
			}
		};

		RollingOaksPlant { placement, kind, stick_material, ball_material, frond_material }
	}
