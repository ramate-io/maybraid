...
use chico_sbs_geometry::{BraidOakTreeSbs, PenmarchTorchSbs, RorysHeadTrainedSbs, VaseTreeSbs};
...
use crate::levantine_scrub::{
	...
	LevantineScrubHedge, LevantineScrubItem, LevantineScrubRoryHead, LevantineScrubTorch,
	LevantineScrubVaseTree,
};
...
impl TerrainSample for ScrubFlatTerrain {
...
impl BuildWithNoise<RorysHeadTrainedSbs> for LevantineScrubRoryHead {
...
impl BuildWithNoise<VaseTreeSbs> for LevantineScrubVaseTree {
...
impl BuildWithNoise<HighBushShootsShape> for LevantineScrubBush {
...
impl BuildWithNoise<PenmarchTorchSbs> for LevantineScrubTorch {
...
impl BuildWithNoise<BraidOakTreeSbs> for LevantineScrubBraidOak {
	fn build_with_noise(&self, noise: NoiseParams) -> BraidOakTreeSbs {
		let config = NoiseConfig::new(noise);
		let height = sample_f32(&config, self.height, 1.0).max(1.8);
		let canopy_spread = sample_f32(&config, self.canopy_spread, 2.0);
		let span = span_fraction(canopy_spread, height);
