...
Orchard `grow_plant` is the woody template. Tuft groves implement `QuantizedPlant` on one wrapper per authored clump/patch const ([`remixed_tuft_plant!`](src/grove/quantized.rs) / [`remixed_blade_tuft_plant!`](src/grove/quantized.rs)), store the `Arc` from `grow_num`, and clone out of that `Arc` only when `merge_collections > 0` (square XZ bins on the grove extent; default `0` shares the unit). [`impl_tuft_grove_lod!`](src/grove/tuft_lod.rs) emits flattened lazy kits ([`flattened_vegetation_scene_chunks`](../vegetation-components/src/lib.rs)), not nested collection hosts. See [`grow_placed_tuft_params`](src/grove/vc_tuft.rs) and [Monster Grass](src/monster_grass.rs).
