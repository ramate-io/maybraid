...
1. Compose plants with [`nest_flattened_plant_chunk`](src/grove/vc_compose.rs), not the unused nested-host helpers in [`placed_host.rs`](src/grove/placed_host.rs). Live groves emit posed kits under the grove host. Isolated `/show` still uses `FlattenedComponentsOnly<PlacedVegetation<T>>`. Playable sticks stay on that host as **one static compound per plant** (`fixed_layers()`, not Host / `LodScene`) — do not unify a tile into one Avian AABB.
...
Woody High and Medium both nest plant kits on the grove host; Low swaps the tile to canopy proxies. The band edge is `factor × tile_radius`. Default preview / vast tiles stay near [`DEFAULT_GROVE_EXTENT_XZ`](src/grove/extent.rs) (100 m, **half-extent radius 50 m**). Factors are doubled from metre targets so `5.0` at a 100 m radius is `10.0` here.
...
- High/Medium nest one lazy kit producer for the plant list, not one host per plant — [`assert_high_medium_nests_plants`](src/grove/woody_checks.rs).
