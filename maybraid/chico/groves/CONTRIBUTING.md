...
1. Compose plants with [`nest_flattened_plant_chunk`](src/grove/vc_compose.rs), not the unused nested-host helpers in [`placed_host.rs`](src/grove/placed_host.rs). Live groves emit posed kits under the grove host. Isolated `/show` still uses `FlattenedComponentsOnly<PlacedVegetation<T>>`.
