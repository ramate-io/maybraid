...
use crate::{GroveKind, PlaygroundConfig, ShowKind};

pub fn ui_config() -> GameCommandUiConfig {
	GameCommandUiConfig {
		title: "Vegetation on terrain — / for commands — Y or F1 drawer".into(),
		empty_console_text:
			"Console: `grove rolling-oaks`, `forest`, `show rock-pile`, `terrain-detail`, `mode character`, `help`".into(),
		root_background: Color::srgba(0.08, 0.16, 0.22, 0.82),
		controls_hint:
			"help — grove <kind> — forest [layering] — show <rock> — terrain-detail [formation] — mode free|character".into(),
	}
}
...
pub(crate) fn sync_command_status_text(
	...
	mut last: Local<
		Option<(GroveKind, i32, u32, i32, Option<String>, Option<String>, Option<ShowKind>)>,
	>,
) {
	...
	let detail_key = config.terrain_detail.map(|spec| spec.key());
	let key = (
		config.grove,
	...
	*last = Some(key);
	status.0 = if let Some(spec) = &config.terrain_detail {
		let formation = spec.formation.map(|k| k.as_kebab()).unwrap_or("throw");
		format!(
			"terrain-detail {formation}  stream-radius={}  terrain-radius={}",
			spec.stream_radius, config.terrain_radius
		)
	} else if let Some(show) = config.show {
		format!(
			"show={}  terrain-radius={}  tile-radius={}",
			show.label(),
			config.terrain_radius,
			config.tile_radius
		)
	} else if let Some(spec) = &config.forest {
