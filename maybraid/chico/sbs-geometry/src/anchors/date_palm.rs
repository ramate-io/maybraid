...
pub struct DatePalmProtoAnchors {
	...
	/// Trunk height as a fraction of [`StrictStalk::stalk_height`] (RFC `0.90`).
	pub trunk_height_fraction: f32,
	/// Trunk joint radius as a fraction of stalk height (RFC `0.025`).
	pub trunk_radius_fraction: f32,
	/// Per-segment length as fractions of `H` (RFC `0.05..0.08`).
	pub segment_length_fraction: (f32, f32),
	/// Vertical growth tolerance in radians (RFC ~2°).
	pub angle_tolerance_radians: f32,
	/// Stacked frond rings at the crown (RFC `6..10`).
	pub ring_count: u32,
...
impl Default for DatePalmProtoAnchors {
	fn default() -> Self {
		Self {
			...
			trunk_height_fraction: 0.90,
			trunk_radius_fraction: 0.025,
			segment_length_fraction: (0.05, 0.08),
			angle_tolerance_radians: 2.0_f32.to_radians(),
			ring_count: 8,
			fronds_per_ring: 12,
			ring_spacing_fraction: 0.01,
...
impl DatePalmProtoAnchors {
	...
	pub fn hysteresis_seeds(&self, chain_noise: NoiseConfig) -> Vec<DatePalmChain> {
		let h = self.height();
		let trunk_r = self.trunk_radius();
		let (len_lo, len_hi) = self.segment_length_fraction;
		let base = BallStickNode::new(self.stalk.stalk_base_anchor, trunk_r);
		...
		let branch = BranchOut::up(base)
			...
			.with_radius_range(trunk_r..trunk_r)
...
mod tests {
	...
	fn default_trunk_segment_count_in_expected_band() {
		...
		assert!((12..=20).contains(&n), "segment count {n}");
	}
}
