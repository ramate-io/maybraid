//! Shared spawn path for merged tuft [`RenderItem`] types.

use bevy::prelude::*;
use render_item::CascadeChunk;

/// Strip non-uniform scale from a spawn transform; return uniform factor for mesh authoring.
pub(crate) trait TuftSpawnTransform {
	fn tuft_spawn_uniform(self) -> (Transform, f32);
}

impl TuftSpawnTransform for Transform {
	fn tuft_spawn_uniform(self) -> (Transform, f32) {
		let s = self.scale;
...
pub(crate) trait MergedTuft: Component + Clone + Send + Sync + 'static {
	...
	fn spawn_render_entities(
		...
		let (root_transform, world_uniform_scale) = transform.tuft_spawn_uniform();
		let root = commands
			.spawn((
				item.clone(),
				cascade_chunk.clone(),
