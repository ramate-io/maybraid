...
**Refresh pass:** [`LodRefreshCorePlugin`](../lod/lib/src/refresh.rs) tracks [`LodNode`](../lod/lib/src/refresh.rs) poses (and mirrors [`LodViewer`](../lod/lib/src/refresh.rs) into [`LodViewerState`](../lod/lib/src/refresh.rs)). Prefer [`LodSceneRefreshPlugin`](../lod/lib/src/refresh.rs) / [`LodFinePhaseAllPlugin`](../lod/lib/src/refresh.rs) for per-host update → fulfill → cull via ephemeral [`LodRef`](../lod/lib/src/lod_ref.rs) + [`LodHostBounds`](../lod/lib/src/refresh.rs). Probe-driven hosts use [`LodRefreshCullPlugin`](../lod/lib/src/refresh.rs). For amortized spawn, use `add_lod_refresh_chunk_full_for::<T>()` / [`scene_chunks_with_level`](../lod/lib/src/gen/presentation.rs) — see [scene chunks](../lod/docs/scene-chunks.md). Cameras are playground-only (`LodViewer` requires `LodNode` on the fly-cam). See also [structural LOD collectors](../lod/docs/structural-lod-collectors.md).
