...
**Refresh pass:** [`LodRefreshCorePlugin`](../lod/lib/src/scene/refresh.rs) tracks [`LodNode`](../lod/lib/src/scene/refresh.rs) poses (and mirrors [`LodViewer`](../lod/lib/src/scene/refresh.rs) into [`LodViewerState`](../lod/lib/src/scene/refresh.rs)). Prefer [`LodSceneRefreshPlugin`](../lod/lib/src/scene/refresh.rs) / [`LodFinePhaseAllPlugin`](../lod/lib/src/scene/refresh.rs) for per-host update → fulfill → cull via ephemeral [`LodRef`](../lod/lib/src/scene/lod_ref.rs) + [`LodHostBounds`](../lod/lib/src/scene/refresh.rs). Probe-driven hosts use [`LodRefreshCullPlugin`](../lod/lib/src/scene/refresh.rs). For amortized spawn, use `add_lod_refresh_chunk_full_for::<T>()` / [`scene_chunks_with_level`](../lod/lib/src/scene/lod_scene.rs) — see [scene chunks](../lod/docs/scene-chunks.md). Cameras are playground-only (`LodViewer` requires `LodNode` on the fly-cam). See also [structural LOD collectors](../lod/docs/structural-lod-collectors.md).
