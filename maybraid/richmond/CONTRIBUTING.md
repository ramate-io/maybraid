...
**Refresh pass:** [`LodRefreshPlugin`](../lod/lib/src/refresh.rs) tracks any [`LodViewer`](../lod/lib/src/refresh.rs) into [`LodViewerState`](../lod/lib/src/refresh.rs), then `add_lod_refresh_all_for::<T>()` / `add_lod_refresh_for::<T, M, I>()` updates levels, fulfills [`LodLevelSpawnRequest`](../lod/lib/src/lod_scene_host.rs), and culls via ephemeral [`LodRef`](../lod/lib/src/lod_ref.rs) + [`LodHostBounds`](../lod/lib/src/refresh.rs). Probe-driven hosts use `add_lod_refresh_cull_for::<T>()` alongside their own level updaters. For amortized spawn, use `add_lod_refresh_chunk_full_for::<T>()` / [`scene_chunks_with_level`](../lod/lib/src/gen/presentation.rs) — see [scene chunks](../lod/docs/scene-chunks.md). Cameras are playground-only (`LodViewer` on the fly-cam). See also [structural LOD collectors](../lod/docs/structural-lod-collectors.md).
