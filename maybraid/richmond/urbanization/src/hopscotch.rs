//! Hopscotch urbanization-kind selection (forest parallel).

use bevy_math::Vec3;
use procedural_common::{BucketThrow, NoiseConfig, NoiseParams, UnitRange};

/// Anchor throw lane (offset from the urbanization-cell center).
...
/// One Hopscotch node: anchor weight, outgoing edges, and the kind it selects.
...
/// Select a node item by Hopscotch from `position` (urbanization-cell center).
