...
- `show connecting-stairwell [--case stacked|narrow-slot|shallow|offset|near-offset|mismatch|opposite|same-y|tall|huge|quarter-turn|long-offset|tiny|short-rise|skew|same-y-l|offset-tall|opposite-offset] [--flight spiral|rectangular-spiral|run-and-landing] [--no-upper-landing] [--slab-thickness M]` — `ConnectingStairwell` inside a vertical shaft (two horizontal openings; gizmos: walk-on edges, walk-off arrows, flight polyline). `--case` and `--flight` are independent. Owned floors (run-in + optional upper landing / kink pads) sit plum: kit top on the walk-on / last-tread plane (panel kits are centered, \(\pm 0.2\,\mathrm{m}\)). `--slab-thickness` (default 0.05 m) is the authored kit thickness.
