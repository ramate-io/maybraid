...
- `show connecting-stairwell [--case stacked|narrow-slot|shallow|offset|near-offset|mismatch|opposite|same-y|tall|huge] [--no-upper-landing] [--slab-thickness M]` — `ConnectingStairwell` spiral inside a vertical shaft (two horizontal openings; gizmos: walk-on edges, walk-off arrows, flight polyline). `--case` swaps pathological shaft pairs. Owned floors (run-in + optional upper landing flush with the last tread) share `--slab-thickness` (default 0.05 m).
