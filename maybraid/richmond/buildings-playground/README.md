...
- `show connecting-stairwell [--case stacked|narrow-slot|shallow|offset|near-offset|mismatch|opposite|same-y|tall|huge|quarter-turn|long-offset|tiny|short-rise|skew|same-y-l|offset-tall|opposite-offset] [--flight spiral|rectangular-spiral|run-and-landing] [--no-upper-landing] [--slab-thickness M] [--tread-fill F] [--lapping-ratio R]` — `ConnectingStairwell` inside a vertical shaft (two horizontal openings; gizmos: walk-on edges, walk-off arrows, flight polyline). `--case`, `--flight`, `--tread-fill`, and `--lapping-ratio` are independent. Owned floors (run-in + optional upper landing / kink pads) sit plum: kit top on the walk-on / last-tread plane (panel kits are centered, \(\pm 0.2\,\mathrm{m}\)). `--slab-thickness` (default 0.05 m) is the authored kit thickness. `--tread-fill` (default 0.4) is tread span as a fraction of the tighter opening half-extent (clamped \(0.2\ldots 0.95\)). `--lapping-ratio` (default 0.55) is preferred going / width, not “how chunky one tread looks”: the fitter keeps ~0.18 m rise. Rectangular-spiral adds rim circuits; run-and-landing is I / L / U from the walk-on sides — extra lapping stacks switchbacks on two corridors, then a landing to the walk-on. Stay in 0.5–0.7 on a ~3 m well; ~1.0–1.2 adds another lap; clamped \(0.2\ldots 2.0\).
- `show pathological-connecting-stairwell-gallery` — labeled grid of the review cases (quarter-turn 0.55 / 1.2, stacked U, tiny, offsets, spiral control). Each cell has a wireframe label box.
