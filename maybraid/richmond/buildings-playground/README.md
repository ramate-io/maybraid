...
- `show rect-ring-floor [--outer-x X] [--outer-z Z] [--inner-x X] [--inner-z Z] [--floor] [--ceiling] [--door-east]… [--opening SPEC]…` — courtyard rectangular ring; wide `Passage`/`Aperture` openings (or `side=`) author broad wall omissions along outer/inner sides
