...
- `show connecting-stairwell [--case stacked|opposite|quarter-turn|tiny|tall|stacked-pair] [--kind circular|rectangular] [--tread-fill F]` — exclusive AABB well (circular helix or wall-hugging rectangular flights). Gizmos: cyan well box, lime walk-on, orange walk-off, yellow landings, magenta last leading
- `show connecting-stairwell-examples [--kind circular|rectangular]` — 4×3 gallery of pathological wells (door pairs, skinny/pancake/needle, min/max fill, stacked pair, one-riser squat)
