...
# Flat quad panel (optional four-edge triangles):
cargo run -p richmond-buildings-playground -- show quad --depth 2 --length 6 --tile-width 1
cargo run -p richmond-buildings-playground -- show quad --depth 2 --length 4 --left 1 --right -0.5 --top 0.5 --bottom -0.25
```
...
- `show quad [--depth D] [--length L] [--tile-width W] [--tile-height H] [--left B] [--right B] [--top B] [--bottom B]` — shepherd's-thatch flat quad; omit any edge base / `--length` for optional regions
