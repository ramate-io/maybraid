...
- `show connecting-stairwell [--case stacked|narrow-slot|shallow|offset|near-offset|mismatch|opposite|same-y|tall|huge] [--flight spiral|rectangular-spiral|run-and-landing] [--no-upper-landing] [--slab-thickness M]` — `ConnectingStairwell` inside a vertical shaft (two horizontal openings; gizmos: walk-on edges, walk-off arrows, flight polyline). `--case` and `--flight` are independent. Owned floors (run-in + optional upper landing flush with the last tread) share `--slab-thickness` (default 0.05 m). Run-and-landing also authors kink pads on the flight.
