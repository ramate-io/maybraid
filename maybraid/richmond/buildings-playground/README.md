...
- `show connecting-stairwell [--case stacked|narrow-slot|shallow|offset|near-offset|mismatch|opposite|same-y|tall|huge] [--no-upper-landing] [--landing-thickness M]` — `ConnectingStairwell` spiral inside a vertical shaft (two horizontal openings; gizmos: walk-on edges, walk-off arrows, flight polyline). `--case` swaps pathological shaft pairs. Upper landing (thin slab flush with the last tread, on the opening rim) is on by default; `--landing-thickness` sets slab thickness (default 0.05 m).
