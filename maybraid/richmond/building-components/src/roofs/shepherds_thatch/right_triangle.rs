...
use crate::partitions::lod::{leaf_mesh_ref_lod, PartitionMeshSet};
...
impl ShepherdsThatchRightTriangle {
	/// LOD host with optional [`MeshRef`] axis mirroring on every tier.
	pub fn scene_with_lod_mirrored(
		lod_ref: &LodRef,
		mirror: Option<MirrorAxis>,
	) -> impl Scene + 'static {
		leaf_mesh_ref_lod(
			RIGHT_TRIANGLE_HIGH.mesh_ref().with_mirror(mirror),
			RIGHT_TRIANGLE_MID.mesh_ref().with_mirror(mirror),
			RIGHT_TRIANGLE_LOW.mesh_ref().with_mirror(mirror),
			lod_ref,
		)
	}
}
