//! Posed kit content for one LOD level root.
//!
//! Domain nodes are the [`lod::LodScene`] hosts (see [`crate::PartitionNode`], …); this
//! module only builds the **content** a level root holds. Host scaffolding itself comes
...
pub fn posed_asset_tier(asset: Option<AssetPath>, transform: Transform) -> impl Scene + 'static {
	posed_scene_ref_tier(asset.map(AssetPath::scene_ref), transform)
}
