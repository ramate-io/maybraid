...
fn rectangular_hip_shared_ridge_and_four_hips() {
	...
	assert_eq!(
		roof.params().halves[0].ridge_line,
		roof.params().halves[1].ridge_line
	);
	...
	for hip in roof.hip_panels() {
		...
		assert!(
			(p.x - r.x).abs() > 0.5,
			"hip base should reach past the ridge inset"
		);
	}
...
fn largest_aperture_wins_per_half() -> anyhow::Result<()> {
	let params = PitchedRoofParams::rectangular_hip(Vec2::new(10.0, 6.0), 4.0, 2.5, 1.5);
	let small = PitchedRoof::pitch_opening(
		&params.halves[0],
		0.4,
		0.4,
		0.6,
	...
	let large = PitchedRoof::pitch_opening(
		&params.halves[0],
		0.6,
		0.5,
		2.0,
...
fn pitch_clip_is_centered_quad_on_face() {
	let half = PitchedRoofParams::rectangular_hip(Vec2::new(10.0, 6.0), 4.0, 2.5, 1.5).halves[0]
		.clone();
